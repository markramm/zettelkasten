...
class TestCascadeMCPServer:
    ...
    def test_kb_search(self, server_setup):
        ...
        result = server_setup['server'].call_tool("kb_search", {
            "query": "immigration"
        })
    ...
    def test_kb_get(self, server_setup):
        ...
        search_result = server_setup['server'].call_tool("kb_search", {
        ...
        if search_result["count"] > 0:
            ...
            result = server_setup['server'].call_tool("kb_get", {
                "entry_id": entry_id,
                "kb_name": "test-research"
            })
    ...
    def test_kb_timeline(self, server_setup):
        ...
        result = server_setup['server'].call_tool("kb_timeline", {
    ...
    def test_kb_create_event(self, server_setup):
        ...
        result = server_setup['server'].call_tool("kb_create", {
            "kb_name": "test-events",
            "entry_type": "event",
        ...
        get_result = server_setup['server'].call_tool("kb_get", {
    ...
    def test_kb_update(self, server_setup):
        ...
        create_result = server_setup['server'].call_tool("kb_create", {
            "kb_name": "test-events",
            "entry_type": "event",
        ...
        # Update it
        update_result = server_setup['server'].call_tool("kb_update", {
            "entry_id": entry_id,
            "kb_name": "test-events",
...
class TestMCPProtocol:
    ...
    def server(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config = CascadeConfig(
                knowledge_bases=[
                    KBConfig(name="test", path=kb_path, kb_type=KBType.EVENTS)
                ],
                settings=Settings(index_path=db_path)
            )
